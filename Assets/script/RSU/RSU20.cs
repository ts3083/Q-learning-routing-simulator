...
public class RSU20 : MonoBehaviour
{
    ...
    private int[] actions_RSU = new int[actionNum] { 15, 14, 19, 25 };

    // RSU14 방향 좌표 저장
    private Vector3[] forward_RSU14 = new Vector3[3] { new Vector3(0, 0, 0), new Vector3(917.22f, 0.427f, 604.645f), new Vector3(915.05f, 0.427f, 606.81f) };

    // RSU15 방향 좌표 저장
    private Vector3[] forward_RSU15 = new Vector3[3] { new Vector3(0, 0, 0), new Vector3(933.754f, 0.427f, 602.52f), new Vector3(931.618f, 0.427f, 602.52f) };

    // RSU19 방향 좌표 저장
    private Vector3[] forward_RSU19 = new Vector3[3] { new Vector3(0, 0, 0), new Vector3(917.49f, 0.427f, 621.224f), new Vector3(917.49f, 0.427f, 623.394f) };

    // RSU25 방향 좌표 저장
    private Vector3[] forward_RSU25 = new Vector3[5] { new Vector3(0, 0, 0), new Vector3(931.565f, 0.427f, 627.503f), new Vector3(933.86f, 0.427f, 627.503f), new Vector3(936.15f, 0.427f, 627.503f), new Vector3(938.41f, 0.427f, 627.503f) };
        ...
        for (int i = 0; i < carListNum; i++)
        {
            ...
            if (carList[i].CompareTag("Q_car"))
            {
                ...
                if (carList[i].GetComponent<Car>().dest_RSU == current_RSU)
                {
                    ...
                    carList[i].GetComponent<Car>().direction = "straight";
                }
                else
                {
    ...
    public Vector3 getPosition(int RSU_num)
    {
        // 차량이 RSU 14에서 온 경우
        if (prev_RSU == 14)
        {
        ...
        else if (prev_RSU == 19)
        {
            ...
            {
                case 14:
                    return forward_RSU14[line_num];     // 오른쪽(시계) 방향으로 135º 회전
                case 15:
                    return forward_RSU15[line_num];       // 왼쪽(반시계) 방향으로 45º 회전
                case 25:
                    line_num = Random.Range(1, 5);
                    return forward_RSU25[line_num];       // 왼쪽(반시계) 방향으로 135º 회전
                default:
                    return forward_RSU14[line_num];
            }
        }
        // 그 이외의 경우(차량이 RSU 25에서 온 경우)
        else
        {
            line_num = Random.Range(1, 3);
            switch (RSU_num)
